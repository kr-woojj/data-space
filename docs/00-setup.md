...
1. 다음 명령을 실행하여 이 저장소를 포크하고 로컬 머신에 복제하세요.
    ...
    ```bash
    gh repo fork kr-woojj/data-space --clone
    ```
...
1. VS Code 내에서 새 터미널을 열고 다음 명령을 실행하여 저장소 상태를 확인하세요.
   ...
   다음을 확인할 수 있어야 합니다. `origin`에 `kr-woojj`가 보이면 포크된 저장소에서 다시 복제해야 합니다.

    ```bash
    origin  https://github.com/<your GitHub ID>/data-space.git (fetch)
    origin  https://github.com/<your GitHub ID>/data-space.git (push)
    upstream        https://github.com/kr-woojj/data-space.git (fetch)
    upstream        https://github.com/kr-woojj/data-space.git (push)
    ```
