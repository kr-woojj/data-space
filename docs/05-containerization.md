...
KR은 선급규칙과 국제협약에 따라 생명과 재산 및 환경을 보호하기 위해 조선 및 해양분야에서 세계를 선도하는 종합적 기술 지원 단체입니다. KR의 연구본부는 기존 고객과 잠재 고객에게 조선해운 산업의 디지털 전환을 지원하기 위한 선박 운항 데이터 관리 웹사이트를 런칭하고자 합니다.
...
1. 아래와 같은 프롬프트를 사용하여 Java 앱의 컨테이너 이미지를 빌드하세요.
    ...
    - 먼저 수행할 모든 단계를 식별하세요.
    - Java 앱은 `java/ship_odms`에 위치합니다.
    - 작업 디렉토리는 저장소 루트입니다.
    ...
    - 컨테이너 이미지에 SQLite 데이터베이스 파일 `ship_odms.db`를 생성하세요. 호스트에서 파일을 복사하지 마세요.
    ```
...
1. `Dockerfile.java`가 생성되면, 다음 프롬프트로 컨테이너 이미지를 빌드하세요.
    ...
    - 컨테이너 이미지 이름으로 `ship-odms-backend`를 사용하세요.
...
1. `Dockerfile.dotnet`이 생성되면, 다음 프롬프트로 컨테이너 이미지를 빌드하세요.
    ...
    - 컨테이너 이미지 이름으로 `ship-odms-frontend`를 사용하세요.
    - 컨테이너 이미지 태그로 `latest`를 사용하세요.
    - 컨테이너 이미지가 제대로 빌드되는지 확인하세요.
    - 빌드가 실패하면 문제를 분석하고 수정하세요.
    ```
...
1. 아래와 같은 프롬프트를 사용하여 Docker Compose 파일을 빌드하세요.
    ...
    - 네트워크 이름으로 `ship-odms`를 사용하세요.
    - Java 앱의 컨테이너 이름으로 `ship-odms-backend`를 사용하세요. 대상 포트는 8080이고 호스트 포트는 8080입니다.
    - .NET 앱의 컨테이너 이름으로 `ship-odms-frontend`를 사용하세요. 대상 포트는 8080이고 호스트 포트는 3030입니다.
