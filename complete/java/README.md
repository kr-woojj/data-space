...
데이터 조회에 대한 완전한 CRUD 작업을 제공하는 선박 운항 데이터 관리 서비스(Ship-ODMS)용 포괄적인 Spring Boot REST API 애플리케이션입니다.

## 프로젝트 개요

다음 사양으로 구축된 프로덕션 준비가 완료된 Spring Boot 애플리케이션입니다:

- **패키지 명**: `com.kr.ship_odms`
- **Artifact ID**: `ship_odms`
- **Group ID**: `com.kr`
- **패키지 타입**: `jar`
- **Java 버전**: OpenJDK 21
- **빌드 도구**: Gradle
...
│   │       └── kr/
│   │           └── ship_odms/
│   │               ├── ShipOdmsApplication.java     # 메인 애플리케이션 클래스
│   │               ├── config/
...
└── test/
    └── java/
        └── com/
            └── kr/
                └── ship_odms/
                    └── ShipOdmsApplicationTests.java # 통합 테스트
```

## 기능

- ✅ 데이터 작업을 위한 완전한 RESTful API
- ✅ 데이터 관리 (생성, 읽기, 업데이트, 삭제)
...
# 대안: JAR 파일을 직접 실행
# java -jar build/libs/ship_odms-0.0.1-SNAPSHOT.jar
```
...
- **데이터베이스 파일**: `ship_odms.db` (자동 생성)
- **위치**: 프로젝트 루트 디렉터리
- **스키마**: Hibernate에 의해 자동 생성
- **샘플 데이터**: `data.sql`에서 로드 (있는 경우)

데이터베이스를 재설정하려면 `ship_odms.db` 파일을 삭제하고 애플리케이션을 다시 시작하세요.
...
```properties
# 애플리케이션 설정
spring.application.name=ship_odms
server.port=8080

# 데이터베이스 구성
spring.datasource.url=jdbc:sqlite:ship_odms.db
...
# JAR 위치
ls -la build/libs/ship_odms-0.0.1-SNAPSHOT.jar
```
...
```bash
# 프로덕션 프로필로 실행
java -jar build/libs/ship_odms-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod

# 또는 커스텀 포트로
java -jar build/libs/ship_odms-0.0.1-SNAPSHOT.jar --server.port=8081
```
